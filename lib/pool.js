 ...
 * @param {Socket=} options.subversion - Subversion (user agent) to share with remote peers
 * @param {Socket=} options.version - Protocol version to share with remote peers
 * @returns {Pool}
 * @constructor
 */
...
Pool.prototype._addPeerEventHandlers = function(peer, addr) {
  ...
  Pool.PeerEvents.forEach(function addPeerEvents(event) {
    peer.on(event, function peerEvent(message) {
      var eventName = 'peer' + event;
      self.emit('*', peer, message, eventName);
      self.emit(eventName, peer, message);
    });
  });
};
